...
| Optional | `ANTHROPIC_API_KEY` | RCA text via `llm-router-service` |
...
```bash
cd /path/to/causr
cp .env.example .env   # edit APP_SECURITY_API_KEY / ANTHROPIC_API_KEY if needed
...
| `ANTHROPIC_API_KEY` | empty | llm-router (RCA skipped if unset) |
...
| No RCA | Set `ANTHROPIC_API_KEY` and run `llm-router-service` |
...
3. Optional Slack on anomaly; optional Claude RCA in the anomaly detail panel  
