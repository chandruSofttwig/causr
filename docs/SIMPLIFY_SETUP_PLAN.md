...
3. **Fewer moving parts in default profile** — demote optional deps (Slack, Groq RCA, legacy UI).
...
| Guided first-run TUI | Asks Slack/Groq once, writes `.env` |
...
| RCA | Opt-in via env; stack healthy without Groq API key |
